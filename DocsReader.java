...
public class DocsReader {
        ...
        if (documentTab != null && documentTab.getBody() != null) {
            ...
            if (bodyElements != null) {
                for (StructuralElement element : bodyElements) {
                    processStructuralElement(element, tabInlineObjects, level + 1, elements);
                }
            }
        }
    ...
    private static void processStructuralElement(StructuralElement element, Map<String, InlineObject> inlineObjects, int sectionLevel, List<ContentElement> elements) {
        if (element.getParagraph() != null) {
            processParagraph(element.getParagraph(), inlineObjects, sectionLevel, elements);
        } else if (element.getTable() != null) {
            processTable(element.getTable(), sectionLevel, elements);
    ...
    private static void processParagraph(Paragraph paragraph, Map<String, InlineObject> inlineObjects, int sectionLevel, List<ContentElement> elements) {
        ...
        for (ParagraphElement pe : paragraph.getElements()) {
            if (pe.getInlineObjectElement() != null) {
                ...
                processImage(objectId, inlineObjects, sectionLevel, elements);
            }
        }
    }

    private static void processTable(Table table, int sectionLevel, List<ContentElement> elements) {
    ...
    private static void processImage(String inlineObjectId, Map<String, InlineObject> inlineObjects, int sectionLevel, List<ContentElement> elements) {
        ...
        // Use the actual image URL from the inlineObject
        ...
        System.out.println("Added image at level " + sectionLevel + ": URL=" + imageUrl);
    }
